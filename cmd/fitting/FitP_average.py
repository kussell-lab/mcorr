...
def calcP2(fBar, thetaC, phiC, d, x):
    ...
    """
    v = (4*(3*fBar*thetaC - 2*thetaC*x + d*(2 + 2*fBar*phiC + 3*thetaC)*x)*(2*thetaC**2*(9*fBar*(2 + 2*fBar*phiC + 3*thetaC) - 18*(1 + fBar*phiC + 2*thetaC)*x - 2*phiC*x**2) + d*thetaC*(2 + 2*fBar*phiC + 3*thetaC)*(-9*fBar*(4 + 2*fBar*phiC + 9*thetaC) + 18*(2 + fBar*phiC + 4*thetaC)*x + 4*phiC*x**2) + d**2*(2 + 2*fBar*phiC + 3*thetaC)**2*(9*fBar**2*phiC + 9*fBar*(1 + 3*thetaC) - x*(9 + 18*thetaC + phiC*x))))/(fBar**2*(6 + 6*fBar*phiC + 9*thetaC - 2*phiC*x)*(3 + 3*fBar*phiC + 9*thetaC + phiC*x)*(d*(2 + 2*fBar*phiC + 3*thetaC)*(9*thetaC + 8*phiC*x) - 2*thetaC*(-6*fBar*phiC + 9*thetaC + 8*phiC*x)))
    return v
