		...
		if l > 0 && ks == 0.0 {
			...
			for i := 0; i+l < len(codonSequences[0]); i++ {
				...
				for _, cc := range codonSequences {
					if i+l < len(cc) {
						//all codonpairs (two codons separated by l) from a list of codon seqeunces
				...
				for _, codonPairs := range multiCodonPairs {
					if len(codonPairs) >= 2 {
						//a codon pair is a pair of codons separated by l
						//i think the double codons are a list of all codon pairs
