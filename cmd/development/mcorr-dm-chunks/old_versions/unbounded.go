...
func main() {
	...
	//distmatrix.Scale(0.5, distmatrix)
...
func csvtodistmatrix(id int, wg *sync.WaitGroup, m *sync.Mutex, file string, strainMap map[string]int, distmatrix *mat.Dense) {
	...
	//initialize matrix for this file
	size := len(strainMap)
	fileMatrix := mat.NewDense(size, size, nil)
	for {
		record, err := r.Read()
		if err == io.EOF {
			break
		...
		if record[5] == "all" {
			...
			pos_i, _ := strainMap[strain1]
			pos_j, _ := strainMap[strain2]
			fileMatrix.Set(pos_i, pos_j, dist)
			fileMatrix.Set(pos_j, pos_i, dist)
			//fileMap[pos_key{pos_i, pos_j}] = dist
			//fileMap[pos_key{pos_j, pos_i}] = dist
		}
	...
	//add values from the fileMatrix to the whole distance matrix
	distmatrix.Add(distmatrix, fileMatrix)
	m.Unlock()
