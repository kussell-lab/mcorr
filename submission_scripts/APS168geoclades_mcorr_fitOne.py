...
archive = "/scratch/aps376/recombo/APS168_SC2_Archive/geo_clades"
submitdir = "/scratch/aps376/recombo/APS168geoclades/%s_submissions" % date
slurmdir = "/scratch/aps376/recombo/APS168geoclades/%s_slurm" % date
...
pairs = np.array_split(complete_combolist, 45)
    ...
    for i in np.arange(0, len(pairs)):
        ...
        with open(job_file, "w+") as fh:
            ...
            for c in pairs_i:
                ...
                fh.writelines("\n")
                if c[0] != c[1]:
                    fh.writelines("mcorr-xmfa-2clades %s/%s/MSA_SC2_MASTER_GAPFILTERED_%s %s/%s/MSA_SC2_MASTER_GAPFILTERED_%s %s/%s_%s_XMFA_OUT &&\n"
                    ...
                    fh.writelines("mcorr-xmfa %s/%s/MSA_SC2_MASTER_GAPFILTERED_%s %s/%s_%s_XMFA_OUT &&\n"
