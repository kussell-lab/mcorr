...
func main() {
	...
	mateAlnFile := app.Arg("second alignment", "Second alignment file in XMFA format").Required().String()
...
func calcP2Coding(aln, mateAln Alignment, codonOffset int, maxCodonLen int, codingTable *taxonomy.GeneticCode, synonymous bool, codonPos int, mateSequence *seq.Sequence) (results []mcorr.CorrResults) {
	...
	//define sequences of the alignment file
	...
	for i, seq1 := range mateCodonSequences {
		for j := 0; j < len(codonSequences); j++ {
			...
			for l := 0; l < maxCodonLen; l++ {
				d := 0.0
				t := 0
				for k := 0; k < len(seq1)-l && k < len(seq2)-l; k++ {
					c1 := seq1[k]
					c2 := seq2[k]
