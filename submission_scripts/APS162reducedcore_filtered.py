...
#define directories
date = "0308reducedfiltered"
jobdir = "/scratch/aps376/recombo/APS162reducedfiltered"
archive = "/scratch/aps376/recombo/APS162_SP_Archive/APS156filteredreduced_threshold90"
submitdir = "/scratch/aps376/recombo/APS162reducedfiltered/%s_submissions" % date
slurmdir = "/scratch/aps376/recombo/APS162reducedfiltered/%s_slurm" % date
    ...
    for i in np.arange(0, len(pairs)):
        ...
        with open(job_file, "w+") as fh:
            ...
            for c in pairs_i:
                ...
                fh.writelines("\n")
                if c[0] != c[1]:
                    fh.writelines("mcorr-xmfa-2clades %s/%s/MSA_REDUCED_CORE %s/%s/MSA_REDUCED_CORE %s/%s_%s_XMFA_OUT &&\n"
                                  % (archive, c[0], archive, c[1], outdir, c[0]+'_'+c[1], g))
                    fh.writelines("mcorrFitOne %s/%s_%s_XMFA_OUT.csv %s/%s_%s_FIT_OUT || true\n"
                                  % (outdir, c[0]+'_'+c[1], g, outdir, c[0]+'_'+c[1], g))
                else:
                    fh.writelines("mcorr-xmfa %s/%s/MSA_REDUCED_CORE %s/%s_%s_XMFA_OUT &&\n"
                                  % (archive, c[0], outdir, c[0], g))
                    fh.writelines("mcorrFitOne %s/%s_%s_XMFA_OUT.csv %s/%s_%s_FIT_OUT || true\n"
